...
import CharTrip from "../Charts/CharTrip";
import MainChartsTop from "./MainChartsTop";
...
const ControlPanel: React.FC = () => {
    ...
    await axios
      .get(
        process.env.NEXT_PUBLIC_NODE_MODE === "development"
          ? `${process.env.NEXT_PUBLIC_API_LOCAL}/users`
          : `${process.env.NEXT_PUBLIC_API_PRODUCTION}/users`,
        {
          headers: {
  ...
  const getCatchData = async () => {
    ...
    await axios
      .get(
        process.env.NEXT_PUBLIC_NODE_MODE === "development"
          ? `${process.env.NEXT_PUBLIC_API_LOCAL}/catch-data`
          : `${process.env.NEXT_PUBLIC_API_PRODUCTION}/catch-data`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  if (
    allUsers.length <= 0 &&
    allUsersActive.length <= 0 &&
    allUsersUnActive.length <= 0 &&
    cashData.count <= 0
  ) {
    return <Loader />;
  }
    ...
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        {cashData && (
          <CardDataStats
            title="Total views"
            total={formatNumber(cashData.count)}
            rate={"10%"}
            levelUp
          >
            <svg
